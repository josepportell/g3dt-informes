...
| 4 | P2a col·lapse (Rubí) | ~2 h | ✅ **FETA 2026-09-06 (tarda, 2), GO Josep:** el col·lapse pren el nivell portant (Rubí Qa 3,0 → 3,5 = signat; `viab` 62 → 69 %). Abans: visible NOMÉS a la variant `viab` (sense lectura): 7/7 cel·les (nom «Gresos… (Nivell 2)», γ 2,20 / c 1,00 / φ 35 / E 500). Amb lectura, `_apply_lectura_soil_levels` tapa la descripció abans d'`is_rock` i només queden Nb «52-R» i E 469. **Acoblada a P2b** (mateixa regla) |
| 5 | **M341** mesura completa (341 vars) | ~1-2 dies | ✅ **v1 FETA 2026-09-06 (vespre)**: `docs/wizard-headless/mesures/mesura_341.py` — els 7 projectes generats NOMÉS amb la lectura (variant `viaA`: escalars de `_decisions.json` per la superposició del wizard, taules llegides, Excel DPSH, Df del signat com a única assumpció) + `t2` als 3 amb dades d'abril; context de plantilla vs `eva_reference_values.json` (`status_for`) per grup A / calc / narr / fix / resta + les 11 taules. Run `runs/2026-09-06-m341`. Destapa: sense sondeig la geometria de nivells surt del segmentador DPSH, no del tall llegit (Linyola/Alcoletge/Vilanova/Anciles: nivell portant i roca equivocats) → **P5** proposada; `detect_soil_type` classifica lutites/rebliment/argiles com a granular/grava. DECISION-LOG 2026-09-06 (vespre) |
| 6 | P2b fondària sabata | ~½ dia | ✅ **càlcul fet 2026-09-06 (tarda, 2), GO Josep; UI FETA 2026-09-06 (vespre)** (nota «Nivell portant: i/n «descripció» · Df = x m» sota «Profunditat fonamentacio», ambre si la Df és el prefill per defecte; `_calc_regime` visible sota φ). Abans: Df del wizard arriba a la tria d'estrat; regla «primer competent a Df + 0,2»; 7/7 signats als tests amb la Df de cada informe. Queda la UI (nivell portant i Df visibles, avís si Df és el prefill) i Linyola/Anciles depenen de la Df dels pous. Bell-lloc Qa 2,5 → 1,5 destapa P3. Abans: `foundation_depth_m` JA és al wizard (prefill 0,3) però `report_data.py:468` passa el 0,8 fix a `_select_bearing_layer_idx`, que tria «el competent més profund». Proposta: «el primer competent que la sabata assoleix (Df + 0,2-0,4)», mai més profund; mesurar Qa abans/després |
| 7 | P3 E per criteris | ~1 dia | ✅ **FETA 2026-09-06 (tarda, 3)** com a candidats amb procedència (`automation/geotech_criteria.py`: γ/c/φ/E + tipus sísmic; rebuig al nivell): Qa 3/3 = signat, φ 11/11, 35/44 cel·les signades exactes i 9 com a candidat. Bell-lloc E 650 i els E de roca per projecte queden com a candidats (pregunta 14 a Eva) |
