...
| 10 | ~~**Camp «peu / font» per a la figura pujada**~~ | ✅ **FET 2026-09-10** (DECISION-LOG 2026-09-10 (2)): a la finestreta de `fig_projecte_1/2` hi ha el camp «Peu de la figura (inclou la font)», prefixat amb el peu actual o «Detall del projecte. Font: G3DT.»; viatja amb la pujada (`caption` del multipart) i «Desa el peu» l'edita al lloc per a la figura actual (`/choose` amb la mateixa entrada: mateixa posició, cap duplicat). | — |
| 11 | ~~**«Guardar» de la pestanya «Imatges» esborra `_lector_selection` i `alternatives`**~~ | ✅ **FET 2026-09-10** (DECISION-LOG 2026-09-10 (2)): `select_photos` conserva la resta del fitxer i guarda la instantània del lector la primera vegada, com `/choose`; la font per ranura de la finestreta continua dient «tria del lector» a les que no s'han tocat. #8 («Restablir selecció IA» esborra el fitxer sencer) continua obert. | — |
| 12 | ~~**Pujades substituïdes**~~ | ✅ **FET 2026-09-10** (DECISION-LOG 2026-09-10 (2)): una pujada que no és a cap ranura (cap cadena de les dues seleccions) s'esborra —fitxer, entrada de l'índex i renders `figsel_*`— després de cada escriptura (`/choose`, `/upload`, «Guardar», «Restablir»). L'original és a l'ordinador de l'Eva; tornar-la a pujar és un clic. | — |
