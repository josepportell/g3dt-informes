...
| 10 | **Camp «peu / font» per a la figura pujada** | Pujada d'imatges (DECISION-LOG 2026-09-10): la figura del projecte pujada per l'Eva porta el peu «Detall del projecte. Font: G3DT.» (decisió del Josep, de moment). Falta un camp —a la pujada o al wizard— perquè l'Eva escrigui el peu i la font que vol que surti. El servidor ja accepta `caption` a l'`entry` de `/choose`. | S |
| 11 | **«Guardar» de la pestanya «Imatges» esborra `_lector_selection` i `alternatives`** | `select_photos` (`web/api.py`) reescriu `photo_selection.json` només amb `source` i les 5 ranures. Amb la pujada hi ha dues vies d'escriptura sobre el mateix fitxer: si l'Eva puja i després desa des de la pestanya, la tria del lector deixa de tornar com a alternativa. Unificar-ho amb `_apply_alternative_choice`. Va amb #8-#9. | S |
| 12 | **Pujades substituïdes** | Es queden a `validation/uploads/imatges/` (i a la pestanya com a candidates). Cap neteja, com l'evidència HITL. Només si molesta. | — |
