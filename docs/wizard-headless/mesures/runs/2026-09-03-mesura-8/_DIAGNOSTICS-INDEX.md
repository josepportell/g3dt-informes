...
| **L3** (1.5) | ✅ (1 cel·la: Anciles MA-1 `n30` ALERTA → OK; `client_name` forma neta) | `consolidate._strip_phone_tail` (a `add_entry`, camps de persona/empresa) + `_ma_sample_has_no_n30` (files SPT/MA, només si totes les etiquetes són MA); `test_L3_*` ×2 | `_reconsolida-2026-09-05-l3/`: taules 144/27/6/20/0; escalars idèntics |
| **L1** (1.5) | ✅ (1 cel·la: Linyola `spt_ma_tests[0].n30` BUIT → CAND «R» = or i signat; re-lectura d'1 document, 1,63 USD) | skill v1.8 (full d'assaig SPT p.3 → fila `spt_ma_tests` amb `registre`); `consolidate._cell_signals` n30: alies `value_candidates`… + «≥ 50 cops → R» derivat; `test_L1_*` | `runs/2026-09-05-l1-linyola/` (lectura nova, copiada al run mare) + `_reconsolida-2026-09-05-l1/`: taules 144/28/6/19/0; escalars idèntics |
| **v1.9** (esmena nit 7) | ✅ validada: annex DPSH de Linyola re-llegit (1,09 USD), `n30: "R"` imprès amb nota «sense registre» | skill v1.9; D3/D4 esteses pels nivells per punt de la banda de color de l'annex (`test_D14_read_base_equal_…`, `test_D14_sample_level_claimed_…`) | `runs/2026-09-05-v19-linyola-dpsh/` + `_reconsolida-2026-09-05-v19/`: 3 cel·les de valor, 0 de veredicte; taules 144/28/6/19/0 |
| **T2** (1.6) | ✅ passada LLM apagada (defecte `python`) després de codificar les 2 regles que aplicava: Castellar `utm_x`/`utm_y` (COORDENADES.txt mana; l'annex dona S-1, anotat), Rubí/Castellar `lab_sample_id` (GTL > annexos; el «P3» del manuscrit és el punt) | `consolidate._FIELD_PRECEDENCE` (+`utm_x`, `utm_y`, `lab_sample_id`) + `_annotate_utm_other_points`; `runner._load_config` defecte `python`; `reconsolida_mesura.py --amb-llm`; `test_T2_*` ×2, runner ×2 | `_reconsolida-2026-09-06-t2/`: 2 cel·les (Castellar `lab_sample_id` CAND → OK; Rubí valor GTL); escalars **119/22/5/1/0 (81 %)**; taules 144/28/6/19/0; conflictes 7 → 3 (només `street_address`); −205…−264 s i −1 USD per projecte |
| R4 + persona/despatx (Eva) | ⏳ | — | ordre del handoff §Bloc 1; preguntes 11-13 a l'Eva |
