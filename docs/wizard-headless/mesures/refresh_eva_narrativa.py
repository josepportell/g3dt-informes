...
Ús:
  ...
  [--projects castellar,rubi] [--all-keys] [--drop clau1,clau2]
...
def refresh(slug: str, apply: bool, all_keys: bool, drop: set[str]) -> list[tuple[str, object, object, bool]]:
    ...
    if apply:
        ...
        if changed:
            old.setdefault("notes", []).append(
                f"refresh_eva_narrativa.py 2026-09-06: {changed} claus narratives re-extretes (prefix primer)"
...
def main() -> int:
    ...
    for slug in [s.strip() for s in args.projects.split(",") if s.strip()]:
        ...
        for k, a, b, allowed in diffs:
            tag = "narr" if allowed else "ALTRES (no s'aplica)"
            print(f"  [{tag}] {k}:\n      abans: {str(a)[:110]!r}\n      ara:   {str(b)[:110]!r}")
    return 0
