# /g3dt-llegir-projecte-preext
Variant experimental de /g3dt-llegir-projecte (v1.3) amb pre-extracció determinista. Mateix protocol de lectura; només canvia COM s'accedeix al document i COM s'escriu el JSON.
...
2. **No executis `fitz`, `xlrd`, `openpyxl`, `extract_msg`, `PIL`, `ls`, `find` ni `md5sum`.** El text de cada pàgina és a `page-N.txt`; la pàgina sencera a `page-N.png` (150 dpi); les meitats a `page-N-left/right.png` o `page-N-top/bottom.png` (220 dpi). Excel: `sheet-i.cells.txt` (`REF<TAB>valor`, per citar `Full!REF`) i `sheet-i.csv`. Correu: `body.txt` + `attachments/` (amb `already_in_folder` al meta si l'adjunt ja és solt a la carpeta: no el llegeixis dos cops). Imatge: `image.png`. El md5 del document és `source_md5` del meta / `md5` de l'inventari.
3. Llegeix el text (`Read` del `.txt`) i DESPRÉS mira les imatges (`Read` dels PNG), com faries obrint el document: sencera per orientar-te, meitats per llegir. Si necessites més resolució en una zona (caixetí, etiquetes petites, manuscrit), usa `.venv/bin/python scripts/render_clip.py PDF --page N --clip x0,y0,x1,y1 --dpi 300-500 --out /tmp/clip.png` (fraccions 0-1) i `Read` del resultat. **Aquest és l'únic codi que executes per mirar.** Si `preext.error` no és null o falta `preext`, torna al procediment normal (fitz) i anota-ho a `reading_notes`.
