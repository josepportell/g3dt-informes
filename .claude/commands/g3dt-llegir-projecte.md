...
T2 (2026-09-06, peça 1.6; el skill de lectura no canvia, continua v1.9): el defecte del runner és `G3DT_LECTURA_CONSOLIDA=python` — NO es crida `--consolida --only-fields` en producció; les dues regles que aquella passada aportava viuen a `consolidate._FIELD_PRECEDENCE` (`utm_x`/`utm_y`: COORDENADES.txt mana; `lab_sample_id`: GTL > annexos de l'Eva, 4/4 signats). El mode `--only-fields` només s'executa amb `G3DT_LECTURA_CONSOLIDA=auto` (mesures). Nota per al Pas 3: la frase «l'annex de l'Eva mana per l'etiqueta de mostra» contradiu el signat de Castellar («MA-1 (S1)» = GTL); el consolidador ja aplica GTL > annex; pregunta 12 a l'Eva oberta.
...
### Mode `--only-fields` (v1.5, Fase 12 — l'única forma de `--consolida` que el runner cridava en producció fins a T2, 2026-09-06; ara només amb `G3DT_LECTURA_CONSOLIDA=auto`)
