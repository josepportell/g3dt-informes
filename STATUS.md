...
| _(25 juny, aquest commit)_ | Via A: DPSH ES 7/7, building_category 5/7, sondeig sense espuris; refactor `_parse_docs_fields` + 17 tests |

Regressió: **32 failed / 1012 passed** (baseline 32-failed inalterat).
...
2. **A3 — extractor positiu de GTL** (Eva ho demana: "no identifica el laboratori"):
   `lab_testing_company`/`lab_location`/`lab_results`. El fix de NIF (24 juny) era la
   meitat negativa; falta extreure el nom del lab (trivial: capçalera PDF / domini email).
3. **A4 / entity confusion**: `architect_company` etiqueta client/promotor com a
...
4. **StreetView adjacents** (Eva ho ha demanat): vista de carrer / Google Earth.
5. **A7 — Wizard UX**: desbloquejar entrada manual de nivells de sòl quan no hi ha
   sondeig_annex (canvi de wizard, separable del pipeline).
6. **Linyola**: pressupost `2_02B_DG_Silvia_Jaume.pdf` (nom no-estàndard); `_find_pressupost_pdf` no el descobreix.
...
1. `docs/DECISION-LOG.md` — entrada 2026-06-25 (llacunes Via A) + 2026-06-24 (fix NIF).
2. `docs/ANALISI-PIPELINE-DEBUG-VACARISSES.md §9` — accions A1–A7 (A2/A8 tancades).
3. `docs/ANALISI-PIPELINE-DEBUG-VACARISSES.md §7` — GTL/laboratori (per A3).
