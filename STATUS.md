...
meitats només per a pàgines sense text + `text_ok` per producer; remesura `preext-v2-c3`. Suite lectura 123 passed. Següent: v2 (mesurada 18:38-19:07, judici pendent de la 2a consolidació) → **run `fable-preext-v2-c3`** (Josep 2026-08-25 vespre: Fable sobre el mateix pipeline; si comparable → Sonnet, si més eficaç/fiable/ràpid → Fable) → Fase 11 → Fase 12 (abans: `fable-c3` després de la 12) (`G3DT_LECTURA_MODEL=fable ~/g3dt-e2e/fase9/run.sh 3 fable-c3`, ~30-40 min:
