...
Last updated: 2026-09-17 (vespre) — **DUES AVARIES SILENCIOSES CORREGIDES I DUES TROBALLES GROSSES OBERTES** (DECISION-LOG 2026-09-17 (2) i (3)). La prova punta a punta amb **Rubí** (projecte mai llegit, 12 docs, `.env` sense cap clau API) va bé: botó «uns 35 min» → real **27 min 04 s**, **12/12 · 0 errors · 0 fallats**, docx 7,4 MB a la xarxa amb **0 marcadors Jinja**, i **0 valors erronis amb confiança** contra el signat (escalars 14 OK/4 CAUTELA/2 ALERTA; taules 15 OK/3 ALERTA/1 CAUTELA/2 BUIT/2 ABSENT — les 5 ALERTA obertes una per una NO són errors). **Corregit (1):** «< 1 min restants» durant els primers **3 min 43 s** d'una lectura de 35 min (`bb10fb6`, a release `37c8c93`, **verificat a l'aplicació real**: ara diu «≈ 20 min» des del segon zero). **Corregit (2):** una **coma decimal a les UTM** feia desaparèixer la coordenada en silenci — l'origen és el **fitxer de camp de la mateixa G3DT** (`COORDENADES.txt` d'Alcoletge: `308781,86 ; 4613950.63`, coma i punt a la mateixa línia), un `input type="number"` no pot contenir-la i la torna buida, i en generar es perdia la consulta geològica a l'ICGC (`649b2b4`). Dues rondes de revisió hi van trobar que la via de producció era `consolidate.py` (no `contract.py`) i que la rèplica JS ja divergia del Python. **OBERT I NO TOCAT (decisió del Josep):** (a) **l'informe de Rubí es contradiu**: «es tracta d'un solar pla» a 3.3.1 amb «4.5 ESTABILITAT DE VESSANT» i pendent real 21,6 % — a Alcoletge no es veia perquè la coma decimal ho tapava, **un defecte n'amagava l'altre**; (b) **18 de 64 camps** de cara a l'Eva arriben decidits pel sistema amb font «automàtica» i **cap compta a «Queden N camps a revisar»** — per això la capa «Observacions de camp» que s'obre sola **no és redundant**, és l'únic avís que existeix. **El patró de fons:** les tres avaries són el mateix error — *el sistema converteix «no ho sé» en una afirmació concreta, en silenci*. Suite: **31 vermells amb els mateixos noms, 2.662 verds**. `production/g3dt-eva-v1` intacta a `123b4f2`. **Data de la visita: el 18-09 es dona per perdut** (l'Eva no respon els correus); objectiu **26-09-2026, sense confirmar**. Handoff per a la sessió següent: `docs/_FOR-NEW-YOU-20260917-1900.md`.

Anterior (2026-09-17, tarda-2) — **PROVA PUNTA A PUNTA AMB RUBÍ (projecte diferent d'Alcoletge): GO** (DECISION-LOG 2026-09-17 (2); run `docs/wizard-headless/mesures/runs/2026-09-17-wsl-rubi-e2e/`). Demanada pel Josep per separar «el sistema funciona» de «funciona amb el cas d'ahir». Rubí, 12 documents, mai llegit al clon, `.env` sense cap clau API: botó «12 documents · uns 35 min» → lectura real **27 min 04 s**, **12/12 · 0 errors · 0 fallats** (confirma en projecte verge la correcció dels duplicats del 16-09), Enllestir 4 s, Generar 17 s, docx 7,4 MB a la xarxa amb **0 marcadors Jinja** i 10 imatges. Qualitat vs signat: escalars 14 OK / 4 CAUTELA / 2 ALERTA / 1 NOU / 1 FORA; taules 15 OK / 3 ALERTA / 1 CAUTELA / 2 BUIT / 2 ABSENT; **erroni amb confiança: 0** — les 5 ALERTA obertes una per una són l'artefacte «prod més confiat que l'or» (a `architect_name` el sistema **no cau al parany** que l'or mateix anota i cita la font; `soil_levels` 3 vs 2 files no és discrepància geològica: tots dos donen `num_soil_levels=1`). UX verificada al navegador: estimació abans del clic, recàrrega en plena lectura, porta suau inline, res visible abans de «Començar». **Troballa T1 trobada i corregida el mateix dia:** «< 1 min restants» durant els primers **3 min 43 s** d'una lectura de 35 min (`_recompute_estimate()` no es cridava a `lectura_inici`; `_round_estimate(0)` no distingia «no ho sé» de «quasi està»). Loop implementer → reviewer (2 rondes, va trobar un segon forat real amb els duplicats i va desmuntar l'alternativa `docs_total > 0`) → tester: suite **31 vermells amb els mateixos noms / 2.632 verds**. **T1 es queda a `experiment/nivell-a-2026-08`; NO fusionada a release** (decisió del Josep pendent) — el clon segueix a `35d4fd3`, o sigui que el run mesura exactament el codi de release. **Data de la visita:** el 18-09 es dona per perdut — l'Eva no ha respost cap dels últims correus; objectiu ara **26-09-2026, sense confirmar**. La icona «Tornar a entrar a Claude» a l'escriptori de l'Eva segueix sent l'únic pendent manual, i ara hi ha marge per fusionar T1 i reverificar sense presses. `production/g3dt-eva-v1` intacta a `123b4f2`.

Anterior (2026-09-17, tarda) — **LES DUES TROBALLES DEL 16-09, CORREGIDES I A RELEASE** (DECISION-LOG 2026-09-17). `experiment` `1ba4924` → **`release/2026-09` `35d4fd3`**, pujat, clon WSL actualitzat. (1) Una sessió de Claude caducada ja no acaba com «✓ Preparat»: és parada amb codi `auth` i la fila diu «Aturat: cal tornar a iniciar sessió a Claude» amb un doble clic a la icona «Tornar a entrar a Claude» (decisió del Josep; drecera nova `scripts/G3DT-Tornar-a-entrar-a-Claude.bat`, amb `claude auth login` — `claude login` NO existeix en aquesta versió). L'enduriment va per un comptador de fallades definitives (`docs_failed`), després que la revisió tombés dues condicions pitjors; de passada es tanca el defecte del comptador que l'Eva veia («19/18», «15/13»). (2) El log «camps canviats per l'Eva» i `_sources` ja no menteixen: base de comparació encadenada, fonts reals preservades, comparació normalitzada. **Verificat de punta a punta amb una sessió realment caducada** al clon: error en 7 s amb el text correcte; després de «tornar a entrar», ready en 4,2 min SENSE reiniciar el wizard; els jobs antics sense la clau nova segueixen «Preparat» net (`runs/2026-09-17-auth-caducada/`). Loop implementer → reviewer (3 rondes) → tester: suite 31 vermells esperats, 286 focalitzats verds. **Pendent el 18-09:** crear la icona a l'escriptori de l'Eva amb el nom exacte (pla §5.6). Seguiments no bloquejants: mode de reserva `projecte` deixa la guarda inerta; falta un test d'integració de reintent; `test_cancellation_mid_flight_kills_live_process` és inestable des d'abans (~30 % amb la màquina saturada).

Anterior (2026-09-16, migdia) — **UX A-G A RELEASE I PAS 2 REPETIT A WSL AMB ALCOLETGE SENCER** (DECISION-LOG 2026-09-16; run `docs/wizard-headless/mesures/runs/2026-09-16-wsl-alcoletge-ux/`). Docs d'ahir committats (`cf2b25f`), merge `--no-ff` d'experiment a **`release/2026-09` → `6134d2a`** (suite curta 167 passats, pujat), clon WSL `~/g3dt-release-wsl-test/app` a `6134d2a` sense cap clau API. Alcoletge (mai llegit al clon): «Començar» → READY en **30,9 min** (botó deia «15 documents · uns 40 min»), 13/13 docs, Enllestir 3 s, Generar 3,5 s, docx a la xarxa (0 Jinja). Lectura vs or: escalars 16 OK / 4 CAUTELA / 0 ERR; taules 18 OK / 4 CAUTELA / 3 ALERTA. Tota la UX A-G vista al navegador (un sol «Començar», recàrrega en plena lectura, «Continuar» sobre job ready, barra i comptador coherents, llegenda, 0 fuites, finestreta d'alternatives, porta suau inline). **Dues troballes que condicionen el 18-09 (decisió del Josep pendent):** (1) sessió de Claude caducada → tots els lectors fallen en 90 s i el job surt com «✓ Preparat» (`_SYSTEMIC_RE` no reconeix «Failed to authenticate»); (2) el log del bloc G «camps canviats per l'Eva» és fals (2 tocats → 5; segon desat sense canvis → 28; `user_data.json` amb 33 `_sources: user`, que muda la precedència Eva > lector). Menors: 15/13 al pas d'imatges, raó repetida a «Figura - Projecte 2», «No UTM coordinates» al generar, Observacions de camp a cada obertura. `production/g3dt-eva-v1` intacta.

Anterior (2026-09-15, nit) — **UX DEL WIZARD, blocs A-G FETS** (`docs/PLA-UX-WIZARD-2026-09.md`; 8 commits `5c63287..c6f82d0`, només `review.html` + 2 tests nous): fora els elements de desenvolupador (Groq, ConceptScout, subetiquetes, «Lectura headless»), banner de format neutre, barra inferior fixa (Generar + desat automàtic + «Queden N camps a revisar» amb porta suau + resultat/error persistent; «Guardar Dades» eliminat), etiquetes de font en llenguatge de l'Eva amb llegenda, stepper plegat, títol «G3DT · Informe geotècnic». Loop implementer→reviewer→tester complet; suite 31 vermells = esperats. E fet (`4e5b755`+esmenes: un sol «Començar»/«Continuar» amb «19 documents · uns 55 min» calculat abans del clic via `GET /api/jobs/estimate/{p}`, sense md5 ni còpia) i G fet (`3577b9c`: log dels camps canviats per desat, font `user_data.json`). 13 commits en total `5c63287..3c9328c`, 51 tests nous, suite 31 vermells = esperats. Pendent: passada al clon WSL amb lectura activa abans del merge a `release/2026-09`. Doc per a l'Eva «Com funciona» (`docs/COM-FUNCIONA-EVA-2026-09.html`, pendent de lectura del Josep).

Anterior (2026-09-10, vespre) — **PREPARACIÓ DEL LLANÇAMENT a l'ordinador de l'Eva, pas 1 FET** (DECISION-LOG 2026-09-10 (3); pla viu: `docs/PLA-RELEASE-2026-09.md`). Branca **`release/2026-09`** al worktree nou `clients/g3dt-release/` = `production/g3dt-eva-v1` (`123b4f2`) + merge d'experiment (2 conflictes resolts amb experiment; índex = experiment + `test_image_cache_name.py`) + `pillow` declarat + CLAUDE.md + egg-info. **M341 des de release: idèntic** a `-nit` (75 % / 82 % / 69 %; condició: la mateixa barreja de carpetes que la referència — 4 des de `reference-material/` amb Fase 0, 3 des de `g3dt-e2e`). **Codi del §10 del handoff fet a experiment i fusionat a release:** els dos lectors d'imatges dins el job (estat `imatges`, pas 4 de 5, Eva mana, empremta per no repetir), aturada per límit d'ús (`error_event usage_limit`, cap fallback silenciós), `pymupdf<1.28` + `requirements-lock.txt` (un venv nou porta 1.28.2 i trenca `render_clip.py`). Suite a release (venv net sense lock): 14 vermells = 11 esperats-amb-dades (els 17 de SmartScan + 3 FileMiner són fixtures: passen amb els fitxers crus) + 3 de pymupdf 1.28; suite amb el venv del lock: **11 vermells / 2579 verds** (només els esperats); **prova en perfil net (Castellar, §10.4): GO** — 13/13 docs, 29,5 min, 0 ERR / 0 ALERTA als escalars, 1 ALERTA de sobreconfiança (valor correcte) i 2 BUIT a les taules, 12,48 $ equiv. (run `2026-09-10-perfil-net-castellar`). **Decisions del Josep (nit):** WSL2 + Ubuntu; pla Max 5x (100 €/mes); les claus API s'absorbeixen amb Claude Code; cita amb l'Eva dilluns 14-09 per al divendres **18-09-2026** (data final la fixa l'Eva). **Pas 2 FET a WSL sense cap clau (DECISION-LOG 2026-09-10 (4)):** clon net + lock + `.env` de l'Eva, Bell-lloc pel navegador: Preparar 45,7 min, Enllestir 2 s, Generar 4 s, lectura 0 ERR, M341 sense claus idèntica a la referència, pujada/peu/tornada OK, 0 crides API, 0 tracebacks. `production/g3dt-eva-v1` no s'ha tocat; `release/2026-09` i experiment pujats a origin.

Anterior (2026-09-10, tarda) — **Pujada d'imatges: els tres pendents FETS** (DECISION-LOG 2026-09-10 (2)) després que el Josep ho provés («la millora és gran»): **#10** camp «Peu de la figura (inclou la font)» a la finestreta de `fig_projecte_1/2` (viatja amb la pujada com a `caption`; «Desa el peu» edita al lloc la figura actual via `/choose` amb la mateixa entrada; pujada sense peu → «Detall del projecte. Font: G3DT.», entrada del lector → «Font: Projecte.»); **#11** «Guardar» de la pestanya «Imatges» conserva `_lector_selection`, `alternatives` i raons (abans reescrivia el fitxer amb 5 claus); **#12** una pujada que no és a cap ranura s'esborra (fitxer + índex + `figsel_*`) després de cada escriptura (`/choose`, `/upload`, «Guardar», «Restablir»). Provat en viu amb Bell-lloc; 3 tests nous; suite **31 vermells amb els mateixos NOMS / 2545 verds / 5 omesos (289 s)**; M341 no repetit (cap canvi al generador). **Commitejat `3b0129a` (codi) · docs (GO «fem els pendents»).** Incident: la meva neteja de l'estat de proves va esborrar dues pujades del Josep a la còpia de Bell-lloc (JSON deixats coherents; memòria `feedback_no_rm_test_state_you_did_not_create`). Han aparegut dos `schemas/formats/learned/*.yaml` (17:24, format learner durant la prova del Josep): NO commitejats, a decidir. Oberts: PENDENTS §3 #8 («Restablir» esborra el fitxer sencer), #9.

Anterior (2026-09-10, matí-tarda) — **IMATGES: l'Eva pot PUJAR una imatge des de la finestreta d'alternatives per a QUALSEVOL ranura de l'informe** (DECISION-LOG 2026-09-10; pla revisat a `docs/imatges/PLA-PUJADA-IMATGES-WIZARD-2026-09-10.md`). Botó «📁 Puja una imatge» a la finestreta (sempre, també amb 0 alternatives) → `POST /api/alternatives/{p}/upload` (valida amb PIL, normalitza EXIF/RGB/≤ 2.400 px, PNG es queda PNG, **nom = ID `YYYYMMDD-HHMMSS-<6 hex>`** a `validation/uploads/imatges/` amb l'original a `pujades.json`) → la mateixa lògica que `/choose` (`_apply_alternative_choice`): `source: user` per ranura, la resta intacta. Cobreix les 5 fotos, assaigs, projecte 1/2 (peu «Detall del projecte. Font: G3DT.», decisió del Josep; camp de peu/font: PENDENTS §3 #10), **situació d'una sola imatge sencera** (150 mm) i **les tres figures automàtiques** (geològic, tall, cullera: claus `geologic`/`tall`/`cullera` a `figure_selection.json`, aplicades després del camí determinista; a la finestreta «Canviar la imatge» / «Tornar a l'automàtic» amb la vista prèvia del calaix). De pas: `/api/photos` llista les pujades (la pestanya ja no pintava buida una ranura pujada), `_entry_ok` exigeix imatge per a tot `kind != project_page`, `_find_composed_geological_map` ignora `validation/`. **Validat en viu amb Bell-lloc** (3.200 × 2.400 EXIF 6 → 1.800 × 2.400 vertical; el `.docx` porta les dues imatges byte a byte; finestreta amb el selector de fitxers real) i **M341 `2026-09-10-m341-pujada` IDÈNTIC a `-nit`** (escalars 315 · 44 · 120 · 31 → 75 %, imatges 31 · 3 · 15 · 7, `fix` 81 M, taules 82 %). 8 tests nous. Suite: 31 vermells amb els mateixos NOMS / 2542 verds / 5 omesos (298 s). **Commitejat `4e45857` (backend) · `2832af7` (wizard) · docs (GO del pla).** Servidor del wizard engegat a `localhost:8765` (sense `G3DT_PROJECTS_DIR`) per si el Josep ho vol provar. Pendents nous: PENDENTS §3 #10-#12. Següent: que l'Eva ho faci servir; preguntes 30-38 amb el Josep.

Anterior (2026-09-09, tarda-4) — **IMATGES, acció 4 FETA: les alternatives dels lectors VISIBLES al wizard sense clicar** (DECISION-LOG 2026-09-09 (4)). Els dos lectors escriuen `alternatives` (fins a 3 candidats per ranura, amb raó); `GET /api/alternatives/{p}` dona per ranura l'actual, les alternatives amb miniatura i raó i «cap font»; `POST …/choose` desa una ranura amb `source: user` conservant la resta i la tria del lector (`_lector_selection`; una foto un forat; el mateix retall no dues vegades; ranura canviable assaigs/projecte; situació doble). Wizard: zona flotant `#altZone` dalt a la dreta (30vw × 30vh, z-index 10000, filla de `<body>`: dins `#dataContent` amidava 0 × 0) que s'obre amb el botó «Alternatives (n)» de cada ranura i es tanca amb l'aspa; **correcció del Josep (nit)**: ni la zona ni les imatges apareixen abans de llançar el pipeline del projecte (`imagesReadyFor`), clic en lloc de hover, pestanya «Imatges» visible en producció (v1 l'amagava), secció «Imatges de l'informe» dins el formulari del Wizard amb els mateixos botons, i el stepper ja no marca «Llest» amb la visió en marxa (DECISION-LOG 2026-09-09 (5)). De pas: el calaix de figures llegeix `figure_selection.json` (claus de la 7a), `FOTOGRAFIA` en singular ja llista fotos, i la cau global es filtra per projecte (a Tulipa el calaix ensenyava el retall d'ANCILES). **Provat pel Josep amb Bell-lloc (lectors passats a la còpia de `~/g3dt-prod-workspace`): set correccions seves aplicades el mateix vespre (DECISION-LOG (4) «Correcció»). Run `2026-09-09-m341-accio4` idèntic a `-tall`; REFERÈNCIA nova `2026-09-09-m341-nit` amb el Cadastre viu: escalars 315 · 44 · 120 · 31 → 75 % (idèntics a `altres-b`), imatges 31 M · 3 C · 15 X · 7 ND (69 %), `fix` 81 M, taules 82 %.** 8 tests nous. Suite: 31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2534 verds / 5 omesos (182 s, Cadastre viu). **Commitejat `06d7541` · `9f9bc91` · docs (GO).** Pendents: `alternatives` als 7 del corpus a la propera passada; «tornar a la tria del lector»; `select_photos_ai` (PENDENTS §3 #7-#9). Següent: `docs/_FOR-NEW-YOU-20260910.md`; que l'Eva ho provi; preguntes 30-38 amb el Josep.

Anterior (2026-09-09, tarda-3) — **IMATGES, acció 3 FETA: estudi del retall del tall als 7 signats** (DECISION-LOG 2026-09-09 (3)). Mesurat mm a mm (figura de l'Eva localitzada per NCC dins `tall.pdf`): de les 4 C, **2 eren estètiques** (marge blanc de 10 mm sobre les etiquetes a Castellar i Rubí, 3-9 als altres, sense regla: es queden, «sempre igual») i **2 un defecte nostre** (Linyola i Vilanova: 49 i 73 mm de llegenda i plànol a sobre de la secció). Causa: el FreeHand exporta com a UN traç línies separades (llegenda + terreny) i la caixa del traç entrava al nucli com un estrat → `_ink_rects` (tinta, no caixa); «blanc» = 5 mm absolut (números de l'eix a 0,4-3,7 mm de la barra, llegenda mai a menys de 16 mm; Bell-lloc tallava «(msnm)» i els números); finestra amunt ≥ 40 mm (etiquetes 15-32 mm sobre els estrats; Alcoletge perdia A-A' amb el nucli net). Cau `tall_crop2`. **Run `2026-09-09-m341-tall`: `fig_tall` 3 M · 4 C → 4 M · 3 C (Linyola ph 30 → 6, Vilanova 32 → 4; Bell-lloc M → C per «(msnm)» sencer, registre #14); imatges 30 → 31 M · 3 C · 15 X · 7 ND (69 %).** Escalars: només `adjacent_intro` a Castellar i Alcoletge, **avaria del Cadastre** («problemas tecnicos», anunciada de 15:15 a 19:00): referència d'escalars continua `altres-b`; taules idèntiques. +3 tests. Suite: 31 esperats + 21 de `test_cadastre_progressive` (mateixa avaria) / 2505 verds. **Commitejat `92bd384` · `04e9c52` · `949e12a` (GO).** Pendent: re-mesura i els 21 tests quan el Cadastre torni. Següent: acció (4) wizard amb alternatives visibles.

Anterior (2026-09-09, tarda) — **IMATGES, acció 1 de l'anàlisi de discrepàncies FETA: els PNG d'`ALTRES`/`OTROS` de l'Eva són candidats del lector de fotos** (DECISION-LOG 2026-09-09 (2)). `lector_fotos.py`: `list_eva_pngs` (qualsevol imatge dins `ALTRES`/`OTROS`), candidats `kind: foto | eva_png` marcats «PNG de l'Eva» després de les fotos, dedupe md5 + phash ≤ 2 (foto ↔ PNG); skill: el criteri és pel que s'hi veu (vista del solar sí, mapa/plànol/tall no; l'annex no hi diu res) — la primera redacció («captura de visor») va fer descartar la captura de Street View de Rubí perquè «sembla una foto». **De pas, l'aparellament foto ↔ annex és invariant a la rotació**: mesurat als 7, 2 de 31 imatges d'annex (la cullera SPT vertical de Rubí i Vilanova) només s'aparellen girades 90°; sense la pista el lector resolia l'empat de materials a l'atzar (M → X → M en tres passades, cadascuna després d'un canvi de mecanisme, runs `lector-fotos-altres`/`-b`/`-c`). La pestanya de fotos del wizard veu els mateixos candidats (`kind: eva_png`). **Run `2026-09-09-m341-altres-b` vs `peca7b`: imatges 29 → 30 M · 4 C · 15 X · 7 ND (69 %), Rubí `foto_vista` ND → M (phash 0) i 83 → 86 %; `fix` 79 → 81 M · 21 X (77 → 79 %); escalars 313 → 315 M (75 %); taules intactes; cap altra cel·la moguda** (Castellar i Vilanova ignoren els 12 i 8 mapes nous a les 3 passades). 5 tests nous. Suite: 31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2523 verds / 5 omesos (224 s). **Commitejat `44c9106` · `aa72d41` · `c2af8a0` (GO).** Següent: acció (3) retall del tall (0 tokens) i acció (4) wizard amb alternatives visibles; preguntes a l'Eva amb el Josep.

Anterior (2026-09-09, migdia) — **IMATGES pas 3, peça 7b FETA: el LECTOR DE FIGURES és Claude Code, amb el skill NEUTRE; el pas 3 d'imatges queda tancat en el que és mecànica** (DECISION-LOG 2026-09-09). `automation/imatges/lector_figures.py` + skill `g3dt-llegir-figures`: candidats = retall de l'annex de l'Eva + pàgines amb dibuix del projecte de l'arquitecte (carpeta d'expedient `NN.NNNN/` i rols) + PNG d'ALTRES, cada un renderitzat amb QUADRÍCULA de fraccions i franja «CANDIDAT N · fitxer · pàgina» (sense la franja el lector descrivia la p11 i escrivia l'índex de la p3); exemplars leave-one-out; una crida `claude -p` (sonnet/medium, 37-95 s); `figure_selection.json` que MANA sobre assaigs i projecte també quan diu «cap» (si no, Bell-lloc imprimia el mateix dibuix dues vegades); situació sempre determinista (regla dels insets provada i retirada: Bell-lloc sí, Alcoletge no). **Intervenció del Josep a mig camí: els 7 signats són SITUACIONS, no objectius; el skill citava projectes pel nom i el lector reproduïa el que l'Eva hi havia fet.** Skill reescrit neutre (`grep` dels 7 noms = 0, també al de fotos) i re-mesurat: **run `2026-09-09-m341-peca7b`: `fix` 77 → 79 M · 23 X (77 %), escalars 311 → 313 M (74 %), taules intactes; imatges 29 M · 4 C · 15 X · 8 ND → 69 %** (in-sample, N=7; amb fuites era 78 % / 68 %, run `-fuites` només com a evidència). Lector amb skill neutre: assaigs M a Castellar, Rubí, Alcoletge i Anciles (phash 6-8), X a Linyola (l'Eva: planta acolorida amb icones seves, irreproduïble) i Vilanova (ncc 0,683), sobrant a Bell-lloc (l'Eva el posa com a projecte: pregunta 31); figures del projecte a Linyola (emplaçament) i Anciles (secció) defensables però no les seves (registre #11-#12, pregunta 32). Registre #8-#10 passen a «oberta». 8 tests nous (`tests/test_peca7b_lector_figures.py`). Suite: **31 vermells amb els mateixos NOMS / 2518 verds / 5 omesos (238 s)**. Memòria nova `feedback_reference_projects_are_situations_not_targets`. **SENSE commit (GO pendent; partició suggerida a la DECISION-LOG).** **Anàlisi de les 27 discrepàncies** (petició del Josep): `docs/imatges/ANALISI-DISCREPANCIES-2026-09-09.md` — 22 de 27 tenen la font a la carpeta; accions per rendiment/cost al §3 (ALTRES al lector de fotos, foto per punt, retall del tall, wizard amb candidats, workflow `ALTRES` per al geològic, preguntes). **Següent:** preguntes a l'Eva (30-32, 34, 37-38, 19a) amb el Josep; wizard amb la tria del lector editable (`source: user`, situació doble inclosa); bloc 5 del PLA (castellà); pendents de plantilla del §3 de `PENDENTS-IMATGES.md`.

Anterior (2026-09-08, nit) — **IMATGES pas 3, peça 7a FETA: la ranura única de figura es parteix en tres blocs amb peu propi i la numeració de les figures va per presència** (DECISION-LOG 2026-09-08 (5); GO del Josep al capítol 2.2 i als tres peus). Plantilla: `{%p if not fig_situacio_image_2 %}` taula d'1 cel·la + «Situació de la zona d'estudi.» / `{%p if fig_situacio_image_2 %}` dues imatges en línia + «Figura N i Figura N+1. Detall de la ubicació de la parcel·la en estudi. Font: Projecte.» (Bell-lloc) / figures del projecte 0-2 amb peu del lector / **la figura d'assaigs al 2.2** (5 de 6 signats), «Situació de l'estructura projectada i els assaigs realitzats.» — script `docs/imatges/scripts/peca7_plantilla.py`, **cap taula nova** (mesurat: una taula més mou 23-28 claus de veritat per projecte, perquè l'extractor aparella les taules per ordre). Generador: `figure_numbers` (situació → projecte → assaigs → cullera → geològic → tall) refet a `render_template` amb les imatges reals; `num_project_figures` fora. `image_manager`: `fig_situacio_image_1/2`, `fig_assaigs_image`, `fig_projecte_image_1/2` + `_caption`, noms antics com a àlies; l'`A.01.pdf` sencer i el retall `main_plan` del rol FORA (D4). Extractor: dos peus poden portar el mateix forat (`fig_situacio_num`); **els forats de numeració han d'acabar en `_num`**. Veritats: +6 claus, −3. **Run `2026-09-08-m341-peca7a`: `fix` 74 → 77 M · 25 X, escalars 308 → 311 M (74 %), taules intactes, l'únic canvi d'estat és Bell-lloc `fig_main_plan_num` X → `fig_situacio_2_num` X; imatges 31·4·12·9 → 29 M · 4 C · 13 X · 10 ND (74 → 72 %), transitori** (Bell-lloc retall sense punts imprès com a assaigs; Vilanova un sol retall per a dues figures; registre #8-#10) fins a la **7b**. **Correcció al handoff 2130: de les 25 X de `fix` només 10 són cascada de figures** (5 = taules de Linyola, registre #2; 7 seccions; 3 fotos): sostre de la peça 7 sobre `fix` 85 %, no 93 %. 14 tests nous (`tests/test_peca7_figures.py`), 4 actualitzats. Suite: **31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2510 verds / 5 omesos (203 s)**. **Següent: 7b, el lector de figures** (skill + `figure_selection.json`; Anciles ×2, Linyola secció + planta amb punts, Bell-lloc insets + classificació, Vilanova retall estret): fins a +7 imatges en M i +10 M de `fix`. Els `.docx` generats: situació a l'1.1, assaigs al 2.2, numeració contínua, cap resta de Jinja.

Anterior (2026-09-08, vespre) — **IMATGES pas 3, peça 4 FETA: la FIGURA DELS ASSAIGS surt del full «plànol de situació» de l'Eva, retallat pel dibuix** (`detect_plan_region` / `crop_plan` a `automation/imatges/retall.py`: nucli = la imatge incrustada més gran del full — els seus fulls tenen sempre dos mapes petits a dalt i el dibuix gran a sota — i el mateix creixement fins al blanc de la peça 3 per agafar-hi les cotes i les etiquetes «P-n» que ella dibuixa a fora; mapes, nord i logo com a zones excloses, caixetí com a límit dur; sense nucli → candidat següent, mai un retall inventat). DECISION-LOG 2026-09-08. **`fig_assaigs` 0 % → 100 % (4 M) i `fig_projecte` 0 % → 67 % (2 M); imatges 17·3·23·13 → 23 M · 3 C · 19 X · 11 ND (47 → 58 %)**; **cap altra cel·la moguda** (escalars idèntics als 7, total 74 %, taules 82 %, `fix` 74 M · 25 X intacte). Els 6 encerts són phash ≤ 10. Gotcha nou: **els fulls de l'Eva són A3 VERTICAL girats 270°** — `get_images`/`get_drawings`/`get_text` donen coordenades sense girar i `page.rect` girades: geometria amb `mediabox` i rectangle final per `page.rotation_matrix` (`detect_section_region` no s'ha tocat). L'ordre de candidats posa la carpeta d'annexos abans que l'arrel: el `pl situ.pdf` de l'arrel és l'export imprimible amb el raster en tires (Vilanova: 1.119 imatges) i no té nucli. Tolerància del creixement 1 % (altiplà 0,5-2 %; al 5 % salta a la llegenda d'Anciles). Run `2026-09-08-m341-peca4`. Suite: **31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2483 verds / 5 omesos (177 s)**. **Queda per a la peça 7: el peu de la figura** (avui la ranura única imprimeix el de Bell-lloc sobre la figura d'assaigs de 4 projectes) **i Linyola** (l'Eva hi va fer servir la planta del projecte, no el seu annex). Següent segons `docs/imatges/PENDENTS-IMATGES.md` §6: retall de PNG → peça 6 (geològic) → peça 5 (situació) → peça 7. **I tot seguit (DECISION-LOG 2026-09-08 (2)): el «retall de PNG» del pendent 2 NO calia** — mesurat, els PNG d'`ALTRES` tenen 2-6 % de marge i el retall no mou cap veredicte; `m7.png`, `F1 UBI.png` i `F1 SIT.png` ja són MATCH **phash 0 crus**. I el tall de Rubí no era un marge: `F5 TALL.png` és un **dibuix diferent** del signat (dos nivells amb llegenda, 204-212, contra el nivell únic amb la cota de fonamentació vermella, 208-213). La causa era que el rol `figure_correlation` passava davant del `tall.pdf`; ara surt de `ROLE_TO_FIGURE_VAR` i es prova com a últim recurs. **`fig_tall` 86 → 100 % (0 X, la primera ranura neta als 7); Rubí 40 → 60 %; imatges → 23 M · 4 C · 18 X · 11 ND (60 %)**, cap altra cel·la moguda. Run `2026-09-08-m341-tall-rubi`. **I la PEÇA 6 (DECISION-LOG 2026-09-08 (3)): el mapa geològic que l'Eva ja ha compost va abans que la recepta ICGC** (`_find_composed_geological_map`: un fitxer d'imatge amb «geol» al nom; als 7 dona exactament un candidat als tres que en tenen i cap als altres quatre). **`fig_geologic` 0 → 50 % (3 M, phash 0: la mateixa imatge bit a bit); imatges → 26 M · 4 C · 17 X · 9 ND (64 %)**; Castellar 71 %, Rubí 67 %, Vilanova 71 % i tots dos últims **sense cap forat d'imatge pendent**; cap altra cel·la moguda. El rol `figure_geological_map` de SmartScan NO serveix (encerta a Rubí i Vilanova, però a Castellar apunta a `M1.png` i a Linyola a una imatge minada d'un correu): tercer rol de figura en dos dies que porta a una imatge que no és la del signat. **Els paràmetres de `get_geological_map_with_terrain` NO s'han tocat** (és la imatge que l'Eva va aprovar) i ara està mesurat per què no serviria: cap buffer de 150 a 1.000 m acosta Linyola/Bell-lloc/Alcoletge (NCC 0,31-0,52 a tot arreu) — **la diferència no és el zoom sinó la vista del visor**, i això ho desbloqueja la **pregunta 34** a l'Eva, no el codi. Anciles queda ND honest (Aragó/IGME, i sense UTM). Efecte lateral volgut: als 3 projectes amb PNG ja no es crida `_download_icgc_images()`, que baixava l'ortofoto i consultava el Cadastre per a res. **Correcció al handoff `_FOR-NEW-YOU-20260907-2000` §4.2**: els 3 sense UTM són Rubí, Vilanova i Anciles, no Linyola/Bell-lloc/Alcoletge. Run `2026-09-08-m341-peca6`. **I la PEÇA 5 (DECISION-LOG 2026-09-08 (4)): la figura de situació són els DOS MAPES del full de l'Eva, de costat** (`detect_situation_maps` / `compose_situation`: els dos mapes petits que hi ha sobre el dibuix de la peça 4, retallats amb el mateix creixement fins al blanc i posats de costat a la mateixa alçada; si SmartScan té `figure_situation_map` i la imatge és AMPLA (≥ 1,5) es fa servir la seva sencera — hi encerta 2 de 2 amb phash 0). **`fig_situacio` 0 → 71 % (5 M); imatges → 31 M · 4 C · 12 X · 9 ND (74 %)**; Castellar 86 %, Rubí 83 %, Vilanova 86 %, Anciles 83 %, Alcoletge 67 %; cap altra cel·la moguda. **`_render_situation_plan_left` (el retall del 38 % esquerre, 7 X als 7 signats) esborrat**, amb un test que li barra la tornada. **Gotcha car: l'ordre dels dos mapes s'ha de decidir amb els rectangles ORIGINALS, abans de créixer** — amb els crescuts, Castellar cau de phash 10 a 36 i Rubí de 6 a 32, perquè el creixement d'un mapa li pot moure la vora per davant de l'altre. Descartat trobar el PNG pel nom (no hi ha patró: `m7.png` / `F1 UBI.png` / `F1 SIT.png`) i aparellar-lo contra la nostra composició (escull malament 2 de 3). Obert: Linyola (phash 18, la mateixa figura amb un pèl més de marge; cap marge fix serveix per als tres alhora) i Bell-lloc (dos retalls del plànol i DUES figures on hi ha una sola ranura). Run **`2026-09-08-m341-peca5` (referència per a la peça 7)**. Suite: **31 vermells amb els mateixos NOMS / 2496 verds (178 s)**. **Imatges 47 % → 74 % en una sessió. Només queda la peça 7** (peus propis, partició de les ranures i numeració variable: també hereta el peu de la figura d'assaigs, `fig_assaigs` de Vilanova i les dues de Bell-lloc).

Anterior (2026-09-07, nit) — **IMATGES pas 3, peça 3 FETA: el TALL DE CORRELACIÓ es retalla** (`automation/imatges/retall.py`: nucli = els farciments amples de color, finestra relativa, creixement que s'atura al blanc; límits durs al caixetí i a les imatges incrustades; sense nucli → pàgina sencera, mai un retall inventat). DECISION-LOG 2026-09-07 (nit), D1-D4. **`fig_tall` 0 M · 5 C · 2 X → 3 M · 3 C · 1 X (0 → 86 %)**; imatges 14·5·24·13 → **17 M · 3 C · 23 X · 13 ND (44 → 47 %)**; **cap altra cel·la moguda** (escalars idèntics als 7, total 74 %, taules 82 %, `fix` intacte). Rubí no hi passa: el rol de SmartScan li dona el PNG que l'Eva va compondre (`F5 TALL.png`), més ample que el del signat. Provades 4 regles i mesurades; la que guanya és la més simple (sense heurístiques de llegenda). Gotcha: a PyMuPDF `box |= r` ignora els rectangles degenerats (les línies verticals de «P-1»): unió i contenció per coordenades. Run **`2026-09-07-m341-peca3` (referència per a les peces 4-7)**. Suite: **31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2477 verds / 5 omesos (163 s)**. Commitejat `f508c75` · `e72fc23` (GO). **Correcció (nit, 2, DECISION-LOG): el senyal de la caixa blava NO és exclusiu del sondeig (la foto de DPSH de Castellar porta un objecte blanc i blau a terra i la d'Anciles una vareta de punta verda) i **NO hi ha cap objecte vermell o taronja col·locat** a cap de les 7 fotos de DPSH — el vermell de Linyola i Alcoletge és la carrosseria de la màquina; el senyal bo és QUÈ és l'objecte, no el color. **RESPOSTA de l'Eva (nit, 3): el senyal és una CAIXA BLAVA DE MOCADORS a terra a la foto de la màquina de SONDEIG; la del penetròmetre no porta cap objecte. Pregunta 39 TANCADA i aplicada al skill (`0601a39`).** **Inventari de tot el que queda d'imatges: `docs/imatges/PENDENTS-IMATGES.md`.** Següent: peça 4 (assaigs: retall del dibuix amb els punts; la font, l'annex «plànol de situació», existeix als 7 projectes).**

Anterior (2026-09-07, vespre) — **IMATGES pas 3, peça 2 FETA: el LECTOR DE FOTOS és Claude Code** (skill `g3dt-llegir-fotos` + `automation/imatges/lector_fotos.py`: full de contacte de la carpeta de fotos, annex de fotografies de l'Eva renderitzat i aparellat per phash, exemplars dels 7 signats amb **leave-one-out**, `has_sondeig`, dedupe per md5; crida `claude -p` amb el runner de la lectura; sortida JSON amb raó, confiança i `cap_font`; precedència **Eva > lector > cau IA > patrons**). DECISION-LOG 2026-09-07 (vespre), D1-D8. **Imatges 12·5·25·14 → 14 M · 5 C · 24 X · 13 ND (40 → 44 %)**: `foto_materials` 6/9 → 7/9 **sense cap X**, `foto_sondeig` 1/3 → 2/3, `foto_vista` 1/4 → 2/4; `foto_dpsh` 4/7 → 3/7 i vista de Vilanova ND → X = **empats sense criteri** (preguntes 37 i 38, al registre). Escalars: 1 cel·la (`photo_site_text` de Vilanova), total 74 %; taules 82 % i `fix` intactes. Lector: 25-65 s i ~0,3 $ per projecte (sonnet/medium, 1 crida). Runs `2026-09-07-lector-fotos-peca2c` i `2026-09-07-m341-peca2` (**referència per a les peces 3-7**). Suite: **31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2473 verds / 5 omesos (179 s)**. Commitejat `c90867d` · `8d53b19` · `5aa7e1c` · `f9d88c2` (GO).

Anterior (2026-09-07, tarda-5) — **IMATGES pas 3, peça 1 FETA (plantilla petita): `fig_aerea` fora (taula de situació d'1 cel·la; peu «Situació de la zona d'estudi»), foto de materials UNA vegada dins el 1r nivell (`loop.first`, peu amb `photo_materials_source`), bloc granulomètric per dades del projecte (pastís de Rubí fora), 8 media morts fora (plantilla 7,4 MB → 119 KB, informes −8,5 MB)** (DECISION-LOG 2026-09-07 (tarda-5), D1-D9). Numeració: `fix` 74 M · 25 X → 74 M · 25 X (+9 −9, PREDIT abans del run: la puja és la peça 7); total 308·43·123·33 → 74 % intacte, taules 82 %; imatges 12·5·25·14 (sobrants 3 → 0, pendents 8 → 5). Trobat: l'extractor de referència aparella TAULES per ordre → la situació es queda com a taula d'1 cel·la (paràgraf = 25 claus de taula desalineades als 7). Veritats: Bell-lloc −`fig_aerea_num`, Alcoletge +`fig_cadastre_num`. Risc nou vist: text fix de Rubí «unitat NMgo» a tots els nivells. Suite: **31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2465 verds / 5 omesos (187 s, amb el run `peca1` en paral·lel)**. **Referència per a les peces 2-7: `2026-09-07-m341-peca1b`** (70 mm; `peca1` = 150 mm). Commitejat `b64fe8e` · `b5cf398` (GO).

Anterior (2026-09-07, tarda-4) — **IMATGES pas 3, peça 0 FETA: M341 mesura cada figura del signat contra la nostra («mateixa font que l'Eva»: M phash · C mateixa font altre retall o girada · X · ND no posem res; un a un; sobrants) i baseline amb el codi quiet** (DECISION-LOG 2026-09-07 (tarda-4)). **Referència per a les peces 1-7: `2026-09-07-m341-peca0b` = 12 M · 5 C · 26 X · 13 ND → 40 %** (situació 0/8, assaigs 0/6, projecte 0/5, geològic 0/7, tall 5 C + 2 X, DPSH 4/7, sondeig 1/3, materials 6/9, vista 1/4; sobrants 3); escalars 308·43·123·33 → 74 %, taules 82 %, presència: idèntics a `-bloc4b`. Coherent amb el pas 1 (12 fotos iguals); Rubí materials era EXIF, no retall. Mòdul `docs/wizard-headless/mesures/imatges_font.py` (+ `--remeasure <run>` sense regenerar), 9 tests nous. Suite: **31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2460 verds / 5 omesos (235 s, amb el run `peca0b` en paral·lel)**. Cap canvi a `automation/` ni a la plantilla. **Commitejat `9b47d4b` · `258bed0` (GO). Següent: peça 1 (plantilla petita: aèria fora, materials una vegada, pastís fora, numeració per presència) contra `peca0b`.**

Anterior (2026-09-07, tarda-3) — **IMATGES pas 2 FET amb el Josep (sense codi a experiment): decisions per tipus de figura (DECISION-LOG 2026-09-07 (tarda-3), D0-D14) i `_cache_name` a producció** (`production/g3dt-eva-v1` @ `123b4f2`, test propi `tests/test_image_cache_name.py`, sense push). Regla: la font és el que l'Eva ja té al projecte (PNG d'ALTRES, annexos FH11/PDF, pàgines del projecte), el lector és Claude Code, ICGC per UTM només de reserva, mai dibuixem punts; plantilla amb figures variables (GO), `fig_aerea` fora, materials una vegada (GO); pastís de Rubí fora de la plantilla (GO), gràfic per dades del projecte quan n'hi hagi. Preguntes 30-34 i 36 al registre (35 dins la 19a), no enviades. MCP plànols activat (retalls: `detect_drawing_region`, `crop_to_label`). **Següent: pas 3 en 8 peces mesurables (PLA bloc 4-bis; handoff `_FOR-NEW-YOU-20260907-1450.md`): 0 mesura per figura + baseline `-bloc4b`, 1 plantilla petita (aèria, materials, pastís), 2 fotos amb el lector (skill + exemplars), 3 tall, 4 assaigs, 5 situació, 6 geològic, 7 projecte.**

Anterior (2026-09-07, tarda-2) — **IMATGES pas 1 FET, sense codi: veritat des dels 7 signats, inventari de 291 candidats (FH11 inclosos), aparellament phash+NCC i 7 fulls Eva | nosaltres | candidats** (`docs/imatges/INVENTARI-I-VERITAT-2026-09-07.md`, §0 resum). Troballes: la plantilla és el signat de Rubí (8,9 MB morts + pastís de granulometria de Rubí dins `{%p if show_granulometric %}` = risc de fabricació); l'Eva posa 2/2/3/3/2/3/4 figures del projecte (situació composta topo+orto, assaigs AMB punts 6/7 — «sense punts» era només Bell-lloc —, cap ortofoto a part); Castellar/Rubí/Vilanova deixen les figures a `ANNEXES/ALTRES` (hash idèntic), la resta són retalls del projecte de l'arquitecte o composicions dins FH11. Nosaltres (bloc 4b): fotos iguals 12/23, 4 errònies; figures 0 iguals; tall 7/7 mateixa font, retall diferent. Direcció (Josep): Claude Code mira i interpreta les imatges; llibreria d'exemplars dels signats (leave-one-out); MCP plànols. **Següent: pas 2 amb el Josep sobre §6-§7 (A/B/C per tipus, preguntes 30-36), després pas 3 (§8). `docs/imatges/` commitejat `d041cd8` · `7163f7b` (GO del Josep).**

Anterior (2026-09-07, vespre) — **BLOC 4 del PLA FET i mesurat: numeració (75 %, cap error de comptar) i imatges (full de control + presència); la cau d'imatges compartia el tall de correlació entre 6 projectes** (DECISION-LOG 2026-09-07 (vespre)). 99 veritats `*_num` als 7 signats (extractor per tipus i un-a-un; ES només capçaleres) → grup `fix` 74 M · 25 X; les 25 X: 10 figures del projecte (plantilla fixa 3), 5 taules Linyola (P23), 5 expansivitat (P28), 2 vistes generals Rubí, 1 empentes Anciles (P29), 2 incoherències del signat. Total 74 % intacte fora de `fix`; 12 informes idèntics. Imatges: 8 pendents (6 sense UTM, 2 sense rol `architect_plan`); **bug de cau** (`tall_tall.jpg`, `planol_A.01.jpg`) arreglat amb `_cache_name` per contingut, verificat per md5 (3 → 0 duplicats); `production/g3dt-eva-v1` té el mateix codi. Obert: 4 imatges de tria/pàgina, retall Linyola, foto de materials repetida per nivell (plantilla). Runs `2026-09-07-m341-bloc4-num` → `-bloc4b` (+ `_IMATGES.md`), `2026-09-07-informe-bloc4b`. Suite 31 vermells idèntics / 2451 verds. **Commitejat en 5 (GO del Josep): `78d8d1d` · `64ce2d9` · `85b9c6b` · `4afcc80` · `ea699ea`.** Imatges a refer figura a figura (Josep no satisfet): pla al handoff `_FOR-NEW-YOU-20260907-1130.md`.

Anterior (2026-09-07, tarda) — **BLOC 3 del PLA FET i mesurat: deriva de l'extractor de referència** (DECISION-LOG 2026-09-07 (tarda)). Bloc 2 commitejat en 4 (`bb1e82b` · `33410c5` · `b23a901` · `d8c7698`). Extractor arreglat per clau abans de re-extreure: taula SPT com a bucle (+ `spt_*` de la 1a fila; Rubí/Linyola/Alcoletge la tenien i t5 «sondeig» se l'enduia), fila portant pel que DIU el signat (`bearing_row_from_text`: últim ordinal de la frase «…tensió de treball de:»; «substrat» = última) → `geomech_*` i **`bearing_layer_idx`** a la veritat (6/7 M contra P2b), taules per etiqueta i assignació global (Anciles Taula 1 ja no és la CTE: 1655,01 M, plantes reals, `cte_*`), capçaleres castellanes i notacions «15-R»/«38º»/«>350», àncora sobre «Qa= {{ qa_value }} …» (`settlement_sentence` 7/7, puntuada pel número). Veritats re-extretes a TOTES les claus (`refresh_eva_narrativa.py --keys all`). **Descobert: Vilanova recolza els POUS al 2n nivell → Df de M341 0,3 → 1,1** (la veritat d'abril tenia la fila 1 i el sistema hi coincidia per un error compartit). **`viaA`: total 73 → 74 % (234·43·98·33), A 77 → 78 %, calc 76 → 77 %, taules 82 % intactes; Alcoletge 66 → 75 (registre #3 resolt), Bell-lloc 84 → 87 (`data_camp_text` X era la veritat), Vilanova 60 → 55 (honest), Anciles 50 → 53.** Pèrdues al registre (#6 `geomech_*` i #7 `settlement_sentence` de Vilanova, mateixa arrel: sorres «areniscas» que l'Eva tracta com a roca tova). Runs de referència: `2026-09-07-m341-bloc3b` (`bloc3-veritats` = només veritats, generador intacte). 16 tests nous (`tests/test_bloc3_extractor.py`); Suite sencera: **31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2431 verds / 5 omesos (333 s, amb les mesures en paral·lel)**. Pregunta 27 a l'Eva. No fet: p60 multi-forat (`*_de` no s'extreuen: pregunta 26), castellà (bloc 5). **Commitejat en 4 (GO del Josep): `229aef6` (extractor + tests) · `f3e977c` (comparadors + M341 + refresc) · `953eec4` (7 veritats) · docs i runs; arbre net.** Següent: bloc 4 (numeració i imatges) en una sessió nova (handoff `_FOR-NEW-YOU-20260907-1015.md`).

Anterior (2026-09-07, migdia) — **BLOC 2 del PLA FET i mesurat: la cua del grup A és FORMAT, no lectura** (DECISION-LOG 2026-09-07 (migdia)). Regla: el wizard conserva el text LLEGIT i el generador imprimeix la forma de l'Eva (`automation/formatting.py`: `format_floor_notation` sobre text lliure («PB (planta baixa) + porxada, sense pis superior» → «Pb+Porxo», «PB+1» → «Pb+1Pp»), `format_area` «1.284», `format_cota` «+188.20», `format_spt_id` «SPT-1»; `automation/honorifics.py` nou: «SR./SRA.» pel nom de pila, empreses sense, dubte = sense; `narrative_criteria`: article del tipus d'edificació i `municipality_de`). **Plantilla del repo amb 4 forats que porten la preposició dins el valor** (`{{ client_de }}`, `{{ building_type_de }}`, `{{ municipality_de }}`, `{{ architect_company_de }}`: abans «d'un», «de », «de l'» fixos → «de Alcoletge», «de l'2 Graus»; text verificat als 7 signats abans i després). Comparadors: «1.284» és 1284 (grups de tres xifres = milers) als dos harnesses. **`viaA`: A 74 → 77 % (81·11·33·16 → 89·7·29·16), total 71 → 73 % (208·41·94·33), taules 81 → 82 % (292·99·88); calc, narrativa i resta idèntics; 12 informes 86 % (Bell-lloc t2 SPT id C → M); CAP pèrdua** (la provisional d'Alcoletge «1.167» ↔ «1167» resolta dins el bloc: registre #5). Castellar A 80 → 85, Rubí 81 → 88, Bell-lloc 80 → 88, Linyola M 14 → 16. Descartats amb motiu: `spt_lithology` (forma curta de l'Eva no derivable), cota d'Anciles (annexos a `PDF_V0/ANEJOS/`, regla I1: lectura, no format). Runs de referència: `2026-09-07-m341-bloc2b`, `2026-09-07-informe-bloc2b` (`bloc2` = abans d'arreglar el comparador de taules). 91 tests nous (`tests/test_bloc2_format_a.py`); Suite sencera: **31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2415 verds / 5 omesos (238 s)**. Pregunta 26 a l'Eva (frase dels antecedents per casos; nom de la portada). **SENSE commit (GO pendent; partició suggerida a la DECISION-LOG).** Següent: bloc 3 del PLA (deriva de l'extractor; re-extreure amb els forats `*_de`).

Anterior (2026-09-07, matí) — **BLOC 1 del PLA FET i mesurat: càlcul i taules dels 4 projectes sense sondeig** (DECISION-LOG 2026-09-07). **P5:** sense sondeig la geometria surt del tall LLEGIT (`sondeig_layers_from_levels`: de/a per nivell, «fins al fons» = obert, cap contacte inventat → Vilanova/Anciles segueixen al segmentador però amb la litologia llegida), gruix de l'últim nivell «fins a la fondària investigada» amb asterisc («0.29*» = signat), la mateixa geometria per a Terzaghi-Peck i les files (`ReportData.sondeig_layers_used`). **P6:** `detect_soil_type` delega a `_classify` (ca/es, lutites = roca, «Arcilla limosa» = argila), topall 3,5 per a grava/arena (Rubí Qa 3,5 = signat); un rebliment mai és «dens» per un rebuig. **CTE:** T-1 per defecte (6/6 signats), «PB+1» = 2 plantes (Castellar C-1). **Rang de taules per TAULES** (`insitu_table_range`, generador + wizard; verificat als `.doc`: Linyola «3, 4 i 5» amb dues taules és incoherència del signat). **Data de signatura** al wizard (`data_signatura`, defecte avui, format «29 d'octubre de 2025»); a M341 és la 2a assumpció documentada. Variant «inferiors a» de l'assentament com a candidat. **`viaA`: calc 66 → 76 %, resta 65 → 87 %, total 67 → 71 % (200·45·98·33), taules 79 → 81 %; A i narrativa intactes.** Linyola 72 → 78 / 88 → 91, Alcoletge 61 → 66 / 73 → 75, Vilanova 53 → 60 / 64 → 69, Castellar 76 → 81, Rubí 73 → 76, Bell-lloc 78 → 81. Pèrdues honestes (registre append-only: `docs/REGISTRE-PERDUES-MESURA.md`): Rubí assentament (1,50 només amb Qa 3,0: pregunta 24), Linyola rang (23), Alcoletge escalars `geomech_*` (veritat = fila 1, bloc 3) i columna N (25b). Runs de referència: `2026-09-07-m341-bloc1b`, `2026-09-07-informe-bloc1b` (`bloc1a` = primera passada amb el crash de la Secció 3, conservat com a evidència). 59 tests nous (`tests/test_bloc1_calc_taules.py`), 216 dirigits verds; **suite: 31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2324 verds / 5 omesos (244 s)**. Preguntes 22-25 a l'Eva. **Commitejat (GO del Josep 09:35): `6d2d79b` (P5) · `eb1c2a4` (P6) · `375b7c9` (CTE, rang, data, variant) · docs, tests i runs; arbre net.** Pèrdues acceptades al registre `docs/REGISTRE-PERDUES-MESURA.md`. Següent: bloc 2 del PLA (cua d'A).

Anterior (2026-09-06, nit, 3) — **PEÇA 3 de la narrativa FETA, candidats «+N» al wizard, plantilla de PRODUCCIÓ arreglada** (petició del Josep 2026-09-07; DECISION-LOG 2026-09-06 (nit, 2); anàlisi §10). Estat del solar per criteri (construcció pròpia via DNPRC de les RC llegides + pendent ICGC; sense UTM cap afirmació topogràfica; variants dels signats com a candidats), assaigs de laboratori del bloc «ASSAIGS REALITZATS» del GTL amb el vocabulari de l'Eva (Castellar i Bell-lloc exactes; GTL llegit en ordre de lectura: Linyola en perdia 2 de 4), bloc de vistes generals CONDICIONAL (peu sencer `{{ photo_site_text }}` dins `{%p if %}`; defecte = fotos triades per l'Eva a la pestanya, si no 0: la màquina torna a ser la Fotografia 1 com a 4/7 signats), accés sense costat carrer → nom de la via llegida (Rubí «carrer de la Miranda» exacte). Wizard: grup «Narrativa» amb `access_street`, `site_condition`, `building_structure_desc`, `lab_tests_text`, `num_site_photos` (llista blanca ampliada, badge «+N» amb procedència, desplegable ample). **Narrativa `viaA` 62 → 61 % (32·30·39·8; CA 65, ES 50): 8 NO_DATA entren, 6 com a MISMATCH (estat del solar = judici de la visita, res fals), MATCH 29 → 32; total 68 → 67 %; taules 78 → 79 %** (només la cel·la del laboratori mou, a millor, als 12 informes de `mesura_informe`). Runs de referència: `2026-09-07-m341-peca3`, `2026-09-07-informe-peca3`. Suite: 31 vermells idèntics / **2265 verds**. **Producció: `production/g3dt-eva-v1` commit `c46bc69`** (capçalera 2.1 recuperada, forat sud repetit fora; worktree temporal; **sense push**). **Castellà: decisió del Josep — sí, però no avui.** **SENSE commit a `experiment` (GO pendent; partició suggerida a la DECISION-LOG).** Preguntes 20-21 a l'Eva. **Commitejat (GO del Josep 21:20): `d7d02e9` (peça 3) · `10423c4` (wizard «+N») · `67fcd12` (docs i runs).** **PLA de tot el que queda: `docs/PLA-QUE-QUEDA-DESPRES-DE-A-B-I-NARRATIVA-2026-09-07.md`** (cap tipus nou: cua d'A 33 X i calc 32 X, taules de Vilanova/Alcoletge/Anciles, numeració i imatges MAI mesurades, castellà). **Següent: bloc 1 = P5/P6 + CTE + rang de taules + data de signatura** (handoff `_FOR-NEW-YOU-20260907-2230.md`); després cua d'A, extractor, numeració/imatges, castellà.

Anterior (2026-09-06, nit, 2) — **NARRATIVA per CRITERI, peces 0 + 1 + 2 FETES i mesurades** (GO del Josep; anàlisi a `docs/ANALISI-NARRATIVA-2026-09-06.md`, decisions a DECISION-LOG 2026-09-06 (nit)). **Narrativa `viaA` 30 % → 23 % (mesura honesta, forat contra forat) → 54 % (fórmules de l'Eva: `automation/narrative_criteria.py`, 3 variables de plantilla a frase sencera, `csn` fora) → 62 % (adjacents des de la PARCEL·LA DEL PROJECTE per les referències llegides, plantes del veí via `pt` del DNPRC, vocabulari i agrupació, accés, ubicació, introducció 2.1.1); català 65 %.** Total escalars+narrativa 58 → **68 %**; per projecte Castellar 77, Rubí 71, Bell-lloc 79, Linyola 72, Alcoletge 62, Vilanova 53, Anciles 51; 11 taules IDÈNTIQUES (78 % / 84 %). Extractor de referència: prefix primer + posició + àncora; veritats refrescades només a les claus narratives (`refresh_eva_narrativa.py`). Trobat i arreglat: la plantilla imprimia dos adjacents sobrers i li faltaven la capçalera 2.1 i la frase d'introducció (**la de producció a l'Eva també**); el generador ignorava `site_condition`/`building_structure_desc` de l'Eva. Suite: 31 vermells idèntics / **2258 verds**. Runs de referència: `2026-09-06-m341-narr2c` (M341), `2026-09-06-informe-narr2` (taules). **Commitejat en 4 (GO del Josep, 20:20): `240a9dd` (mesura + extractor + veritats) · `c2582ae` (fórmules + plantilla) · `9dd329d` (adjacents + accés + ubicació) · `284f76a` (docs); arbre net. Següent: peça 3 (estat del solar, laboratori, fotos), candidats al wizard, preguntes 16-19, castellà (decisió), P5/P6, deriva de l'extractor fora de la narrativa.**
