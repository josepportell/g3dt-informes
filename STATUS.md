...
Last updated: 2026-09-07 (tarda-4) — **IMATGES pas 3, peça 0 FETA: M341 mesura cada figura del signat contra la nostra («mateixa font que l'Eva»: M phash · C mateixa font altre retall o girada · X · ND no posem res; un a un; sobrants) i baseline amb el codi quiet** (DECISION-LOG 2026-09-07 (tarda-4)). **Referència per a les peces 1-7: `2026-09-07-m341-peca0b` = 12 M · 5 C · 26 X · 13 ND → 40 %** (situació 0/8, assaigs 0/6, projecte 0/5, geològic 0/7, tall 5 C + 2 X, DPSH 4/7, sondeig 1/3, materials 6/9, vista 1/4; sobrants 3); escalars 308·43·123·33 → 74 %, taules 82 %, presència: idèntics a `-bloc4b`. Coherent amb el pas 1 (12 fotos iguals); Rubí materials era EXIF, no retall. Mòdul `docs/wizard-headless/mesures/imatges_font.py` (+ `--remeasure <run>` sense regenerar), 9 tests nous. Suite: **31 vermells amb els mateixos NOMS que `suite-vermells-esperats.txt` / 2460 verds / 5 omesos (235 s, amb el run `peca0b` en paral·lel)**. Cap canvi a `automation/` ni a la plantilla. **SENSE commit (GO pendent; proposta en 2). Següent: peça 1 (plantilla petita: aèria fora, materials una vegada, pastís fora, numeració per presència) contra `peca0b`.**
